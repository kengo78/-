...
```go
func (レシーバーの変数名 レシーバーの型) メソッド名(パラメータリスト)戻り値の型 {
    //処理
...
//myType型をレシーバーに持つ関数、すなわちmyType型のメソッドを宣言
...
##### メソッドのレシーバーをポインタにする
メソッド内では、レシーバ変数を使ってメソッドを呼び出した対象の値にアクセスすることができます。ただし、その値をメソッド内で変更するには、レシーバの方の前に「＊」を記述して、ポインタにする必要がある。
ポインタにしないと、メソッド内で値を変更しても実際には値のコピーに対して変更を加えているだけなので、メソッドの呼び出し元にはその変更が反映されない。
レシーバの方として初めからポインタ型ではある型をつかうことはできない。

```go
// ポインタ型をレシーバとして使用すると、コンパイルエラーとなる例
type IntPointer *int
// ポインタ型をレシーバにしたメソッド
func (ip IntPointer) x() {
}
```
